...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.product') . '/' . $tring . '.html';
    return $getSlug;
}
/**
 * get size name.
...
function getProductById($id)
{
    ...
    $getSlug = '/' . get_lang() . '/' . trans('routes.product') . '/' . $slug_product;
    return $getSlug;
}
/**
 * get slug product.
...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.trade_mark') . '/' . $tring . '.html';
    return $getSlug;
}
/**
 * get slug product.
...
function getSlugProduct($tring)
{
    ...
    if ($tring != '') {
        $getSlug = '/' . get_lang() . '/' . trans('routes.product') . '/' . $tring;
        return $getSlug;
    } else {
        return $tring;
    }
...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.blog') . '/' . $tring;
    return $getSlug;
}
/**
...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.terms_of_use') . '/' . $tring;
    return $getSlug;
}
/**
...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.cat_blog') . '/' . $tring;
    return $getSlug;
}
/**
...
{
    $getSlug = '/' . get_lang() . '/' . trans('routes.promotion') . '/' . $tring;
    return $getSlug;
}
/**
 * get Name Product.
 ...
 * @return string $slug
 */
function getNameProduct($lang_code, $id)
{
    if ($lang_code == 'en') {
        $name_peoduct = App\Models\Uni_Product::where('id', $id)->pluck('name_en')->first();
    } else {
 ...
 * @return string $slug
 */
function getSlugProductById($lang_code, $id)
{
    if ($lang_code == 'en') {
        $slug_product = App\Models\Uni_Product::where('id', $id)->pluck('slug_en')->first();
    } else {
        $slug_product = App\Models\Uni_Product::where('id', $id)->pluck('slug')->first();
    }
 ...
 * @return int $strength
 */
function generate_code_5()
{
    $input = "0123456789abcdefghiklmnopqrsuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    $strength = 5;
...
{
    $adminName = App\Models\User::where('id', $id)->pluck('name')->first();
    return $adminName;
}
/**
 * get price.
 ...
 * @return int price
 */
function getPrice($lang, $id)
{
    if ($lang == 'en') {
        $configuration = App\Models\Configuration::first();
        $currency = $configuration->currency;
        // $result = App\Models\Product_Size::where('product_id', $id)->pluck('price')->first();
        $result = App\Models\Product_Size::where('product_id', $id)->pluck('price');
        $val = $result[0];
        $n = count($result);
        for ($i = 1; $i < $n; $i++) {
            if ($val < $result[$i]) {
        ...
        $sizePrice = round($val / $currency, 2);
    } else {
        $result = App\Models\Product_Size::where('product_id', $id)->pluck('price');
        $val = $result[0];
        $n = count($result);
        ...
        $sizePrice = number_format($val, 0, '', '.');
    }
    return $sizePrice;
}
function getPriceFe($id)
{
    $result = App\Models\Product_Size::where('product_id', $id)->pluck('price');
    $val = $result[0];
    $n = count($result);
 ...
 * @return int price
 */
function getPriceSaleStoreFe($id)
{
    $result = App\Models\Product_Size::where('product_id', $id)->pluck('price_sale_store');
    $val = $result[0];
    $n = count($result);
    for ($i = 1; $i < $n; $i++) {
        if ($val < $result[$i]) {
...
function getPriceSaleFe($id)
{
    $result = App\Models\Product_Size::where('product_id', $id)->pluck('price_sale');
    $val = $result[0];
    $n = count($result);
    for ($i = 1; $i < $n; $i++) {
        if ($val < $result[$i]) {
 ...
 * @return int price
 */
function getPriceVnd($id)
{

    $result = (int)App\Models\Product_Size::where('product_id', $id)->pluck('price')->first();
...
function getPriceSale($lang, $id)
{
    ...
    //     $sizePrice = round($result / $currency, 2);
    // } else {
    //     $result = App\Models\Product_Size::where('product_id', $id)->pluck('price_sale')->first();
    //     $sizePrice = number_format($result, 0, '', '.');
    // }
    return $sizePrice;
}
 ...
 * @return int price
 */
function getPriceNoFlashSale($lang, $id)
{
    if ($lang == 'en') {
        $configuration = App\Models\Configuration::first();
        $currency = $configuration->currency;
        $result = App\Models\Uni_FlashSale::where('id', $id)->pluck('price_nosale')->first();
        $sizePrice = $result / $currency;
    } else {
        $sizePrice = App\Models\Uni_FlashSale::where('id', $id)->pluck('price_nosale')->first();
    }
    return $sizePrice;
}
/**
 ...
 * @return int price
 */
function getPriceFlashSale($lang, $id)
{
    if ($lang == 'en') {
        $configuration = App\Models\Configuration::first();
        $currency = $configuration->currency;
        $result = App\Models\Uni_FlashSale::where('id', $id)->pluck('price')->first();
        $sizePrice = $result / $currency;
    } else {
        $sizePrice = App\Models\Uni_FlashSale::where('id', $id)->pluck('price')->first();
    }
    return $sizePrice;
}
/**
 ...
 * @return int price
 */
function getPriceSaleStore($lang, $id)
{
    if ($lang == 'en') {
        $configuration = App\Models\Configuration::first();
        $currency = $configuration->currency;
...
/**
 * get data by data by lang.
 * @param string $lang
 * @param string $field
 * @param string $field_en
 * @return string string
 */
function getDataByLang($lang, $string, $string_en)
{
    if ($lang == 'en') {
        $result = $string_en;
    } else {
...
/**
 * get data by data by lang.
 * @param string $lang //language page
 * @return int $vndfm
 */
function formatVndProduct($lang, $price)
{
    if ($lang == 'en') {
        $configuration = App\Models\Configuration::first();
        $currency = $configuration->currency;
        $sizePrice = round($price / $currency, 2);
    } else {
        $sizePrice = number_format($price, 0, '', '.');
    }
    return $sizePrice;
}
/**
 ...
 * @return int $vndfm
 */
function formatVndCart($lang, $price)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == 'en') {
 ...
 * @return int $vndfm
 */
function formatCartVnd($lang, $price, $lang_cart, $product_id, $size_id, $type = '')
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == $lang_cart) {
        ...
        if ($lang_cart == 'en') {
            if ($type == 'combo') {
                $vndfm = round(($price / $currency), 2);
            } else {
                $vndfm = getPriceCart($product_id, $size_id);
            }
            $vndfm = getPriceCart($product_id, $size_id);
        } else {
            $vndfm = round(($price / $currency), 2);
 ...
 * @return int $vndfm
 */
function formatCartVndStore($lang, $price, $lang_cart, $product_id, $size_id)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == $lang_cart) {
        ...
        if ($lang_cart == 'en') {
            $vndfm = getPriceStoreCart($product_id, $size_id);
        } else {
            $vndfm = getPriceStoreCart($product_id, $size_id) / $currency;
        }
    }
    return $vndfm;
}
 ...
 * @return int $vndfm
 */
function formatCartVndSCombo($lang, $price, $lang_cart, $product_id)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == $lang_cart) {
        ...
        if ($lang_cart == 'en') {
            $vndfm = getPriceComboCart($product_id);
        } else {
            $vndfm = getPriceComboCart($product_id) / $currency;
        }
    }
    return $vndfm;
}
 ...
 * @return int $vndfm
 */
function formatPdfVnd($lang, $price)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == 'en') {
...
/**
 * replace price by lang.
 * @param string $lang //language page
 * @return int $vndfm
 */
function formatPriceVnd($lang, $price)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == 'en') {
 ...
 * @return int $vndfm
 */
function formatPriceInterface($lang, $price)
{
    $configuration = App\Models\Configuration::first();
    $currency = $configuration->currency;
    if ($lang == 'en') {
...
/**
 * get data by data by lang.
 * @param int $price //language page
 * @return int $vndfm
 */
function getPriceCart($id, $size_id)
{
    $price_sale = (int)App\Models\Product_Size::where('product_id', $id)->where('size_id', $size_id)->pluck('price_sale')->first();
    $price = (int)App\Models\Product_Size::where('product_id', $id)->where('size_id', $size_id)->pluck('price')->first();
    if ($price_sale != 0) {
        return $price_sale;
    } else {
        return $price;
 ...
 * @return int $vndfm
 */
function getPriceStoreCart($id, $size_id)
{
    $price_sale_store = (int)App\Models\Product_Size::where('product_id', $id)->where('size_id', $size_id)->pluck('price_sale_store')->first();
    $qty_in_box = (int)App\Models\Product_Size::where('product_id', $id)->where('size_id', $size_id)->pluck('qty_in_box')->first();
    $min_box = (int)App\Models\Product_Size::where('product_id', $id)->where('size_id', $size_id)->pluck('min_box')->first();
    $price_store = $price_sale_store * $qty_in_box * $min_box;
    return $price_store;
}
/**
 * get data by data by lang.
 ...
 * @return int $vndfm
 */
function getPriceComboCart($id)
{
    $price_sale_combo = (int)App\Models\Uni_FlashSale::where('id', $id)->pluck('price')->first();
    return $price_sale_combo;
}
/**
 * get data by data by lang.
 * @param int $price //language page
...
function get_link_blank_byname($product_name)
{
    ...
    return '/' . trans('routes.product') . '/' . $link;
}
