        ...
        <div class="col-lg-8">
            <div class="tab-content" id="pills-tabContent">
                @foreach (Config('app.locales') as $key => $lang)
                    <div class="tab-pane fade {{ $lang_code == $lang ? 'show active' : '' }}"
                        id="pills-home_{{ $lang }}" role="tabpanel"
                        aria-labelledby="pills-home-tab_{{ $lang }}">
                        <div class="card box-shadow-0">
                            <div class="card-body pt-3">
                                <div class="form-group">
                                    <label for="exampleInputEmail1"
                                        class="required">{{ __('uni_pages.' . $lang . '.heading_title_name') }}<span>(*)</span></label>
                                    <input type="text" class="form-control keypress-count"
                                        
                                        value="{{ $lang == 'vi' ? old('p_name', $pages->p_name ?? '') : old('p_name_en', $pages->p_name_en ?? '') }}"
                                        data-slug=".slug{{ $lang == 'vi' ? '' : '_' . $lang }}"
                                        name="p_name{{ $lang == 'vi' ? '' : '_' . $lang }}">
                                    @if ($errors->first($lang == 'vi' ? 'p_name' : 'p_name_en'))
                                        <span
                                            class="text-danger">{{ $errors->first($lang == 'vi' ? 'p_name' : 'p_name_en') }}</span>
                                    @endif
                                </div>
                                <div class="form-group">
                                    ...
                                    @if ($errors->first($lang == 'vi' ? 'p_slug' : 'p_slug_en'))
                                        <span
                                            class="text-danger">{{ $errors->first($lang == 'vi' ? 'p_slug' : 'p_slug_en') }}</span>
                                    @endif
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1"
                                        class="required">{{ __('uni_pages.' . $lang . '.form_description') }}
                                        <span>(*)</span></label>
                                    <input type="text" class="form-control keypress-count"
                                    ...
                                    @if ($errors->first($lang == 'vi' ? 'p_desscription' : 'p_desscription_en'))
                                        <span
                                            class="text-danger">{{ $errors->first($lang == 'vi' ? 'p_desscription' : 'p_desscription_en') }}</span>
                                    @endif
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1"
                                        class="">{{ __('uni_pages.' . $lang . '.form_style') }} <span>(*)</span> </label>
                                        <input type="
                                        text" class="form-control"
                                        name="p_style{{ $lang == 'vi' ? '' : '_' . $lang }}"
                                        value="{{ $lang == 'vi' ? old('p_style', $pages->p_style ?? '') : old('p_style_en', $pages->p_style ?? '') }}">
                                        @if ($errors->first($lang == 'vi' ? 'p_style' : 'p_style_en'))
                                            <span
                                                class="text-danger">{{ $errors->first($lang == 'vi' ? 'p_style' : 'p_style_en') }}</span>
                                        @endif
                                </div>
                                <div class="form-group">
                        ...
                        <div class="card  box-shadow-0">
                            ...
                            <div class="card-body pt-3 box-seo hide">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control meta_desscription{{ $lang == 'vi' ? '' : '_'.$lang }} keypress-count" 
                                    data-desscription-seo=".meta_desscription{{ $lang == 'vi' ? '' : '_' . $lang }}"
                                    data-lang-de="{{ $lang == 'vi' ? '' : '_'.$lang }}" name="p_desscription_seo{{ $lang == 'vi' ? '' : '_'.$lang }}"  value="{{ $lang == 'vi' ? old('p_desscription_seo', $pages->p_desscription_seo ?? '') : old('meta_desscription_en', $pages->p_desscription_seo_en ?? '') }}">
            ...
            <div class="card  box-shadow-0">
                <a href="{{ route('get_admin.content_page.create', $pages->id) }}" class="btn btn-xs btn-success"><i
                        class="la la-file-import"></i>Thêm mới</a>
            </div>
            <div class="card  box-shadow-0">
                <div class="card">
                    <div class="card-body">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ $item->thumbnail }}" target="_blank"
                                                    ...
                                                    <img style="height: 100%;border-radius: 10px;border: 1px solid #dedede;width: 100%"
                                                        src="{{ pare_url_file($item->thumbnail) }}" alt="">
                                                </a>
                                            </td>
                                            <td>
                                                <a href="javascript:;"
                                                    title="{{ $item->order }}">{{ $item->order }}</a>
                                            </td>
                                            <td>
                                                <a href="{{ route('get_admin.content_page.edit', $item->id) }}"
                                                    class="btn btn-xs btn-info"><i class="la la-edit"></i></a>
                                                <a href="{{ route('get_admin.content_page.delete', $item->id) }}"
