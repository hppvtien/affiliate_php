...
class AdminUniCategoryController extends AdminController
{
    ...
    public function store(AdminUniCategoryRequest $request)
    {
        ...
        if ($request->icon_thumb) {
            $data['thumb_desscription'] = $this->processUploadFile($request->thumb_desscription);
        } 
    ...
    public function update(AdminUniCategoryRequest $request, $id)
    {
       ...
       if($request->thumb_desscription){
        $data['thumb_desscription'] = $this->processUploadFile($request->thumb_desscription);
        Storage::delete('public/uploads_Product/'.$uni_category->thumb_desscription);
       }else{
        $data['thumb_desscription'] = $uni_category->thumbthumb_desscriptionnail;
       }
