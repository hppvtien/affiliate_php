...
<main id="maincontent" class="page-main"><a id="contentarea" tabindex="-1"></a>
    <div class="page-title-wrapper">
        <h1 class="page-title">
            <span class="base" data-ui-id="page-title-wrapper">{{ trans('title.cart') }}</span>
        </h1>
    </div>
    ...
    <div class="columns">
        <div class="column main"><input name="form_key" type="hidden" value="GXhjnhZzwPqQ9aXV">
            <div class="cart-container">
                ...
                <div class="cart-summary"><strong class="summary title">{{ trans('title.cost_info') }}</strong>
                    ...
                    <div id="cart-totals" class="cart-totals">
                        ...
                        <div class="table-wrapper">
                            <table class="data table totals">
                                <tbody>
                                    <tr class="totals sub">
                                        <th class="mark" scope="row">{{ trans('title.total_amount') }}</th>
                                        <td class="amount">
                                            <span class="price" data-th="Subtotal">{{ formatVndCart($lang_code,(int)\Cart::subtotal(0,0,'')) }} {{ trans('title.currency') }}</span>
                                        </td>
                                    </tr>
                                    @if (get_data_user('web','type') == 2 && checkUidSpiceClubPay(get_data_user('web')))
                                    <tr class="totals sub">
                                        <th class="mark" scope="row">{{ trans('title.endow_spice') }}</th>
                                        <td class="amount">
                                            <span class="price" data-th="discounttotal">-{{ formatVndCart($lang_code,(int)\Cart::total(0,0,'')*(getDiscount()[0])/100) }} {{ trans('title.currency') }}</span>
                                        </td>
                                    </tr>
                                    @else
                                    ...
                                    <tr class="grand totals">
                                        <th class="mark" scope="row">
                                            <strong>{{ trans('title.total_amount_temp') }}</strong>
                                        </th>
                                        <td class="amount" data-th="Order Total">
                                            @if (get_data_user('web','type') == 2 && checkUidSpiceClubPay(get_data_user('web')))
                                            <strong><span class="price">{{ formatVndCart($lang_code,((int)\Cart::subtotal(0,0,'') - (int)\Cart::total(0,0,'')*(getDiscount()[0])/100)) }} {{ trans('title.currency') }}</span></strong>
                                            @else
                                            <strong><span class="price">{{ formatVndCart($lang_code,(int)\Cart::subtotal(0,0,'')) }} {{ trans('title.currency') }}</span></strong>
                                            @endif
                                    ...
                                    <tr class="grand totals">
                                        <th colspan="2" class="title ">
                                          <i> {{ trans('title.total_amount_all_vat') }} </i>
                                        </th>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <ul class="checkout methods items checkout-methods-items">
                        <li class="item">
                            <a href="{{ route('get_user.pay',app()->getLocale()) }}" type="button" data-role="proceed-to-checkout" title="Proceed to Checkout on Co-Op Market" class="a-btn a-btn--primary action primary checkout">
                                <span>{{ trans('title.make_pay') }}</span>
                            </a>
                        </li>
                    </ul>
                </div>

                <form action="{{ route('get_user.updatecart',app()->getLocale()) }}" method="get" id="form-validate" class="form form-cart">
                    <div class="table-responsive">
                        <table id="shopping-cart-table" class="cart items data table cart-table">
                            <caption class="table-caption">{{ trans('title.info_cart') }}</caption>
                            <thead>
                                <tr>
                                    <th scope="col"><span>{{ trans('title.price') }}</span></th>
                                    <th scope="col"><span>{{ trans('title.unit_price') }}</span></th>
                                    <th scope="col"><span>{{ trans('title.quantity') }}</span></th>
                                    <th scope="col"><span>{{ trans('title.total_amount') }}</span></th>
                            ...
                            <tbody class="cart item">
                                ...
                                <tr>
                                    <td data-th="Item" class="">
                                        <a href="{{ getSlugProduct(getDataByLang($lang_code,$item->slug,$item->slug_en)) }}" title="{{ getDataByLang($lang_code,$item->name,$item->name_en) }}" tabindex="-1" class="product-item-photo">
                                            <span class="product-image-container" style="width:110px;">
                                                <span class="product-image-wrapper" style="padding-bottom: 90%;">
                                                    <img class="product-image-photo" src="{{ $item->options->images }}" max-width="100%" max-height="100%" alt="{{ getDataByLang($lang_code,$item->name,$item->name_en) }}">
                                                </span>
                                            </span> <br>
                                            <span class="font-weight-bold">{{ getDataByLang($lang_code,$item->name,$item->name_en) }}</span> <br>
                                            @if($item->options->sale != 'combo')
                                            <span>{{ trans('title.weight') }}:  <b>{{ getSizeName($item->weight) }} / {{ getSizeNameType(round($item->weight,0)) }}</b></span>
                                            @endif
                                        </a>
                                    </td>
                                    <td data-th="Price">
                                    <span class="price-excluding-tax" data-label="Excl. Tax">
                                            <span class="cart-price">
                                                <span class="price" id="hihihihi" gia="{{ $item->price }}">
                                                    {{ formatVndCart($lang_code,$item->price) }} {{ trans('title.currency') }}
                                                </span>
                                            </span>
                                        </span>
                                    </td>
                                    ...
                                    <td data-th="Tổng tiền">
                                        <span class="price-excluding-tax" data-label="Excl. Tax">
                                            <span class="cart-price">
                                                <span class="price" id="price{{ $item->id }}">{{ formatVndCart($lang_code,$item->price * $item->qty) }} {{ trans('title.currency') }}</span>
                                            </span>
                                        </span>
                                    </td>
                                    <td data-th="Xóa sản phẩm">
                                        <button style="width: 20px;height: 20px;padding: 0;background: red;" data-row="{{ $item->rowId }}" type="button" data-url="{{ route('get_user.deletecart',app()->getLocale()) }}" name="remove_cart_action" title="Remove Shopping Cart" class="a-btn a-btn--primary action remove remove_cart_action">
                                            <span>x</span>
                                        </button>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colspan="5" style="padding-right: 0" class="border-bottom-0 border-left-0 border-right-0">
                                        <button type="submit" class="btn btn-info float-right" id="update-cart" style="margin-left:10px">{{ trans('title.update_cart') }}</button>
                                        <button type="button" class="btn btn-info float-right" data-type="del_cart" id="delete-all-cart" data-url="{{ route('get_user.deleteAllCart',app()->getLocale()) }}"><i class="fa fa-spinner-third"></i>{{ trans('title.delete_cart') }}</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="cart main actions">
                        <div class="cart-actions">
                            <a href="{{ route('get.all_product') }}" type="button" name="update_cart_action" title="Update Shopping Cart" class="a-btn a-btn--primary action update update_cart_action">
                                <span>{{ trans('title.continue_shopping') }}</span>
                            </a>
                        </div>
                    </div>
                </form>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{ trans('title.are_you_ok_chacnge') }}</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary close-btn" data-dismiss="modal">{{ trans('title.close') }}</button>
                <button type="button" class="btn btn-success" id="save-qty-product">{{ trans('title.change') }}</button>
            </div>
        </div>
    </div>
</div>
