                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ formatPdfVnd($lang_code,$item->price) }}</td>
                                <td>{{ formatPdfVnd($lang_code,$item->qty * $item->price) }}</td>
                                {{-- <td>{{ getVatProduct($item->id) }}%</td> --}}
                                {{-- <td>
                                    @if ($item->options->product_vat)
                                    {{ formatPdfVnd($lang_code,$item->options->product_vat) }}
                                    @else
                                        0
                                    @endif
                                </td> --}}
                                <td>{{ formatPdfVnd($lang_code,$item->qty * $item->price + $item->options->product_vat) }}</td>
                                
                            </tr>
                            @endforeach
                            <tr>
                                <td colspan="6">Phí Ship</td>
                                <td>{{ formatPdfVnd($lang_code,$data_pdf->total_ship) }} </td>
                            </tr>
                            <tr>
                                <td colspan="6">Ưu đãi</td>
                                <td>{{ formatPdfVnd($lang_code,$data_pdf->total_discount) }} </td>
                            </tr>
                            <tr>
                                <td colspan="5">TỔNG CỘNG TIỀN THANH TOÁN (Đã bao gồm VAT nếu có)</td>
                                <td>{{ formatPdfVnd($lang_code,$data_pdf->total_no_vat) }} </td>
                                {{-- <td></td> --}}
                                {{-- <td>{{ formatPdfVnd($lang_code,$data_pdf->total_vat) }} </td> --}}
                                <td>{{ formatPdfVnd($lang_code,$data_pdf->total_money) }} </td>
                            </tr>
                           <tr>
