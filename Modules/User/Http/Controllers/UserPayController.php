...
class UserPayController extends UserController
{
    ...
    public function getPaySuccsess(Request $request)
    {
        ...
        foreach ($listCarts as $key => $item) {
            if ($item->options->sale != 'combo') {
                \Cart::update(
                    $key,
                    [
                        'price'=> getPriceCart((int)$item->id,$item->weight)
                    ]
                );
            } else {
                \Cart::update(
                    $key,
                    [
                        'price'=> getPriceComboCart((int)$item->id)
                    ]
                );
            }
        ...
        $order_data = [
            ...
            'combo_id' => $combo_id,
            'status' => 0,
            'total_vouchers' => (int)formatPriceVnd($lang_code,$request->total_vouchers),
            'total_money' => formatPriceVnd($lang_code,$request->total_all_pr) == 0 ? (int)formatPriceVnd($lang_code,$request->total_cart) : (int)formatPriceVnd($lang_code,$request->total_all_pr),
            'total_vat' => (int)formatPriceVnd($lang_code,$request->total_vat_cart),
            'total_no_vat' => (int)formatPriceVnd($lang_code,$request->total_all_cart),
            'total_discount' => (int)formatPriceVnd($lang_code,$request->total_discount),
            'total_ship' => (int)formatPriceVnd($lang_code,$request->fee_ship),
        ...
        if ($idOrder) {
            ...
            $url = '/'.$lang_code.'/thanh-toan/' . $idOrder;
            return $url;
        }
    }
    public function getSuccsess(Request $request,$locale, $id)
    {
    ...
    public function processSuccsess(Request $request, $id)
    {
        $lang_code = Session::get('website_language') == null ? 'vi' : Session::get('website_language');
        if($lang_code == 'en'){
            $message = 'Your order has been sent to the system';
        } else {
    ...
    public function returnvnpay(Request $request)
    {
        ...
        if ($request['vnp_ResponseCode'] == '00') {
            ...
            $order->fill($data)->save();
            if($lang_code == 'en'){
                $message = 'Thank you for trusting and supporting us!!!';
            } else {
    ...
    public function getmomoSuccsess(Request $request, $id)
    {
        ...
        $order = Uni_Order::find($id);
        // dd($viewData);
        return view('user::pages.pay.momosuccsess', compact('order','lang_code'));
    }
